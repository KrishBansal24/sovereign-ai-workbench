...
class SovereignTUI(App[None]):
    ...
    async def refresh_backend(self) -> None:
        try:
            ...
            self.render_home(health, docs, artifacts, jobs); self.render_documents(); self.render_data_sources(); self.render_artifacts(); self.render_jobs(jobs); self.result("#knowledge-summary", f"{len(indexed)} documents indexed  •  {sum(1 for item in docs if item.get('index_eligible') and item['document_id'] not in self.indexed_ids)} ready to add")
    ...
    def _handle_deliverable(self, form: dict[str, str] | None) -> None:
        if form: self.create_deliverable(form)
    def open_deliverable(self, kind: str | None = None) -> None: self.push_screen(DeliverableModal(kind), self._handle_deliverable)
    ...
    async def clear_artifacts(self) -> None:
        try:
            self.result("#artifacts-result", "[b]CLEARING ARTIFACTS[/b]\nDeleting all managed artifacts from the library...")
            await self.client.clear_artifacts()
            self.refresh_backend()
            self.result("#artifacts-result", "[b]SUCCESS[/b]\nAll generated artifacts have been cleared.")
        except BackendError as exc:
            self.error("#artifacts-result", exc)
    @work(exclusive=False)
