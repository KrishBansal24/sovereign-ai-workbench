...
def test_chunking_and_overlap(monkeypatch):
    ...
    monkeypatch.setattr(module, "settings", SimpleNamespace(rag_chunk_size=20, rag_chunk_overlap=5))
    chunks = ChunkingService().chunk("doc", "A" * 45)
    assert len(chunks) == 3
    assert chunks[0].chunk_id == "doc:0"


def test_vector_store_persists_searches_and_removes(tmp_path):
    store = VectorStoreService(tmp_path)
    store.replace_document("d1", [[1, 0], [0, 1]], [{"document_id":"d1","filename":"pump.txt","chunk_id":"d1:0"}, {"document_id":"d1","filename":"pump.txt","chunk_id":"d1:1"}])
